...
public class PaySheetsResource {
     ...
     */
    @RequestMapping(value = "/paySheetss",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<PaySheetsDTO> createPaySheets(@RequestBody PaySheetsDTO paySheetsDTO) throws URISyntaxException {
        log.debug("REST request to save PaySheets : {}", paySheetsDTO);
        if (paySheetsDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("paySheets", "idexists", "A new paySheets cannot already have an ID")).body(null);
        }
        PaySheetsDTO result = paySheetsService.save(paySheetsDTO);
        return ResponseEntity.created(new URI("/api/paySheetss/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("paySheets", result.getId().toString()))
            .body(result);
    }
    ...
    @RequestMapping(value = "/paySheetss",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<PaySheetsDTO> updatePaySheets(@RequestBody PaySheetsDTO paySheetsDTO) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert("paySheets", paySheetsDTO.getId().toString()))
            .body(result);
    }
     ...
     */
    @RequestMapping(value = "/paySheetss/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<PaySheetsDTO> getPaySheets(@PathVariable Long id) {
        ...
        return Optional.ofNullable(paySheetsDTO)
