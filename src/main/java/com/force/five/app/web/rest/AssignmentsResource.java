 ...
 */
@RestController
//@Api(value = "Assignments", description = "update and delete Assignments")
@RequestMapping("/api")
public class AssignmentsResource {
	 ...
	 */
	@RequestMapping(value = "/assignmentss", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<AssignmentsDTO> createAssignments(@RequestBody AssignmentsDTO assignmentsDTO)
		...
		if (assignmentsDTO.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("assignments", "idexists",
					"A new assignments cannot already have an ID")).body(null);
		}
		...
		return ResponseEntity.created(new URI("/api/assignmentss/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("assignments", result.getId().toString())).body(result);
	}
	...
	@RequestMapping(value = "/assignmentss", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<AssignmentsDTO> updateAssignments(@RequestBody AssignmentsDTO assignmentsDTO)
			throws URISyntaxException {
	 ...
	 */
	@RequestMapping(value = "/assignmentss", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	 ...
	 */
	@RequestMapping(value = "/assignmentss/{id}", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
	@Timed
	public ResponseEntity<AssignmentsDTO> getAssignments(@PathVariable Long id) {
		...
		return Optional.ofNullable(assignmentsDTO).map(result -> new ResponseEntity<>(result, HttpStatus.OK))
	...
	@RequestMapping(value = "/assignmentss/{id}", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
